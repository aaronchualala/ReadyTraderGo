...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int], ask_volumes: List[int], bid_prices: List[int], bid_volumes: List[int]) -> None:
        ...
        # decision-making for etf_ask-etf_bid-price-arbitrage
            # check spread
            # send fill and kill order to both current bid and ask prices+volumes
            # maintain a var to reserve the volume sent by above orders and below strat should assume reserved volume is not to be touched
            # unreserve the volume if fill and kill didn't work (on order status)
        ...
        stat_sigf = 0.3
        # sell logic
        significant_etf_overpricing = (midpoint_price_of_etf/midpoint_price_of_future-1)/0.003546 > stat_sigf
        in_long_etf_position = (self.position >= 0 and (midpoint_price_of_etf/midpoint_price_of_future-1)/0.003546 > stat_sigf - stat_sigf*(self.position/100))

        should_sell = \
            significant_etf_overpricing or in_long_etf_position
        
        # buy logic
        significant_etf_underpricing = (midpoint_price_of_etf/midpoint_price_of_future-1)/0.003546 < - stat_sigf
        in_short_etf_position = (self.position < 0 and (midpoint_price_of_etf/midpoint_price_of_future-1)/0.003546 < - stat_sigf - stat_sigf*(self.position/100))
